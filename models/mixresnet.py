...
__all__ = [
    'mixres18_w1234a234', 'mixres50_w1234a234', 'mixres18_w1234a234_100', 
    'mixres50_w1234a234_100', 'mixres18_w248a248_chan', 'mixres50_w248a248_chan',"mixres50_w2468a2468_100"
]
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, conv_func, inplanes, planes, stride=1,
        ...
        self.conv1 = conv3x3(conv_func, inplanes, planes, stride, groups=1, **kwargs)
        self.bn1 = nn.BatchNorm2d(planes, affine=bnaff)
        self.conv2 = conv3x3(conv_func, planes, planes, groups=1, **kwargs)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, conv_func, inplanes, planes, stride=1,
        ...
        self.conv1 = conv_func(
            inplanes, planes, kernel_size=1, bias=False, groups=1, **kwargs)
        self.bn1 = nn.BatchNorm2d(planes, affine=bnaff)
        self.conv2 = conv_func(
            planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, groups=1, **kwargs)
        self.bn2 = nn.BatchNorm2d(planes, affine=bnaff)
        self.conv3 = conv_func(
            planes, planes * self.expansion, kernel_size=1, bias=False, groups=1, **kwargs)
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv_func(self.inplanes, planes * block.expansion, kernel_size=1,
                          stride=stride, bias=False, groups=1, **kwargs),
    ...
    def split_complexity_loss(self):
        ...
        for m in self.modules():
            if isinstance(m, self.conv_func):
                ...
                # if layer_idx in [4, 14, 27, 46]:
                if layer_idx in [0, 3, 46]:
...
def mixres18_w1234a234_100(**kwargs):
    return ResNet(BasicBlock, qm.MixActivConv2d, [2, 2, 2, 2], num_classes=100, wbits=[1, 2, 3, 4], abits=[2, 3, 4],
                  share_weight=True, **kwargs)
...
def mixres50_w1234a234_100(**kwargs):
    return ResNet(Bottleneck, qm.MixActivConv2d, [3, 4, 6, 3], num_classes=100, wbits=[1, 2, 3, 4], abits=[2, 3, 4],
                  share_weight=True, **kwargs)


def mixres18_w248a248_chan(**kwargs):
    return ResNet(BasicBlock, qm.MixActivChanConv2d, [2, 2, 2, 2], num_classes=100, wbits=[2, 4, 8], abits=[2, 4, 8],
                  share_weight=True, **kwargs)

def mixres50_w248a248_chan(**kwargs):
    return ResNet(Bottleneck, qm.MixActivChanConv2d, [3, 4, 6, 3], num_classes=100, wbits=[2, 4, 8], abits=[2, 4, 8],
                  share_weight=True, **kwargs)



def mixres50_w2468a2468_100(**kwargs):
    return ResNet(Bottleneck, qm.MixActivConv2d, [3, 4, 6, 3], num_classes=100,
                     wbits=[2, 4, 6, 8], abits=[2, 4, 6, 8], share_weight=True, **kwargs)

# if __name__ == '__main__':
#     model = mixres50_w1234a234()
#     data = torch.randn(1, 3, 224, 224)
#     y = model(data)
#     image = make_dot(y, params=dict(model.named_parameters()))
#     image.format = 'png'
#     image.render('mixres50_w1234a234')
if __name__ == "__main__":
    model =  mixres18_w1234a234_100()
