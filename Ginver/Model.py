...
class InversionModelEffnet(nn.Module):
...
# class InversionModelEffnet_test(nn.Module):
#     def __init__(self, input_channels, output_channels,image_size):
#         super().__init__()
#         self.decoder = nn.Sequential(
...
class InversionModelEffnet_test(nn.Module):
    def __init__(self, input_channels, nc, ngf, nz, ig):
        super().__init__()
        self.nc = nc  # 3
        self.ngf = ngf  # 128
        self.nz = nz  # 530
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(input_channels, 8*128, 4, 2, 1),
            nn.BatchNorm2d(8*128),
            nn.Tanh(), # 8*128*8*8
