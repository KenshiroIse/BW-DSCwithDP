...
def save_checkpoint(state, is_best, epoch, step_epoch, filename='checkpoint.pth.tar',save_dir=datetime.datetime.now().strftime('%Y%m%d_%H%M%S')):
    save = f'./main_model_output/{save_dir}'
    os.makedirs(save, exist_ok=True)
    torch.save(state, f"{save}/{filename}")
    if is_best:
        shutil.copyfile(f"{save}/{filename}", f'{save}/model_best.pth.tar')
    if (epoch + 1) % step_epoch == 0:
        shutil.copyfile(f"{save}/{filename}", f'{save}/checkpoint_ep{epoch + 1}.pth.tar')
...
if __name__ == '__main__':
    ...
    args = parser.parse_args()
    save_dir = f'./model_output/{args.save_dir}'
    os.makedirs(save_dir, exist_ok=True)
    with open(f'{save_dir}/model_output.txt', 'w') as f:
        sys.stdout = f
        main()
    sys.stdout = original_stdout
    print("Program main.py execution is complete.")
