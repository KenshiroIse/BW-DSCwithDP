...
from utils import get_natural_bottlenecks
...
parser.add_argument('-j', '--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers (default: 4)(max: os.cpu_count())')
...
parser.add_argument('--save-dir','--save-dir', default="", type=str, help="save dir")
parser.add_argument('--split-complexity-decay', '--csd', default=0, type=float,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif "effnet" in args.arch:  # crop_size, short_size同じでいいのか
    ...
    bottlenecks = get_natural_bottlenecks(model, input_size=crop_size)
    print(bottlenecks)
    bottlenecks_list = [bottleneck['cnn_layer_number'] for bottleneck in bottlenecks]
    print(bottlenecks_list)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, arch_optimizer, epoch, args, bottlenecks_list)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best, epoch, args.step_epoch, save_dir=args.save_dir)

    print('Best Acc@1 {0} @ epoch {1}'.format(best_acc1, best_epoch))


def train(train_loader, model, criterion, optimizer, arch_optimizer, epoch, args, bottlenecks_list):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        output = model(images)
        # clamped_outputは何のためにある？
        clamped_output = torch.clamp(output, max=100000)
        loss = criterion(output, target)
        # print(loss)   #確認用
        # measure accuracy and record loss
        acc1, acc5 = accuracy(output, target, topk=(1, 5))
        # complexity penaltyを追加する前に更新しちゃだめじゃない？
        ...
        if args.complexity_decay != 0:
            if hasattr(model, 'module'):
                ...
                # print(model.module.complexity_loss()) #確認用
            else:
                loss_complexity = args.complexity_decay * model.complexity_loss()
                # print(model.complexity_loss()) #確認用
            loss += loss_complexity
        
        # split complexity penalty
        if args.split_complexity_decay != 0:
            if hasattr(model, 'module'):
                loss_split_complexity = args.split_complexity_decay * model.module.split_complexity_loss(bottlenecks_list)
                # print(model.module.split_complexity_loss()) #確認用
            else:
                loss_split_complexity = args.split_complexity_decay * model.split_complexity_loss(bottlenecks_list)
                # print(model.split_complexity_loss()) #確認用
            loss += loss_split_complexity
...
# 保存の仕方はoriginalと少し違う
def save_checkpoint(state, is_best, epoch, step_epoch, filename='arch_checkpoint.pth.tar',save_dir=datetime.datetime.now().strftime('%Y%m%d_%H%M%S')):
    save = f'./arch_output/{save_dir}'
    os.makedirs(save, exist_ok=True)
    torch.save(state, f"{save}/{filename}")
    if is_best:
        shutil.copyfile(f"{save}/{filename}", f"{save}/arch_model_best.pth.tar")
    if (epoch + 1) % step_epoch == 0:
        shutil.copyfile(f"{save}/{filename}", f'{save}/arch_checkpoint_ep{epoch + 1}.pth.tar')

# AverageMeterと ProgressMeterはなんのためにある？
...
if __name__ == '__main__':
    ...
    print("Program serch.py execution is complete.")
